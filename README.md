...
flowchart LR
    ...
    p -.迭代推进.-> c
    c -.迭代推进.-> n
    note right of n: 三指针重复，直到 cur 为空<br/>反转完成后 prev 即新表头
```
...
```mermaid
flowchart LR
    b_i[外层 i] --> b_j[内层 j 比较 a[j], a[j+1]]
    b_j --> b_cmp{a[j] > a[j+1]?}
    b_cmp -- 是 --> b_swap[交换]
    b_cmp -- 否 --> b_next[继续]
    b_swap --> b_next --> b_i
    note right of b_next: 冒泡：最大值每轮沉到末尾
```

```mermaid
flowchart LR
    ins[cur 元素] --> shift{前一元素>cur?}
    shift -- 是 --> move[右移空出位置]
    move --> shift
    shift -- 否 --> place[插入到空位]
    place --> nextCur[处理下一个元素]
```

```mermaid
flowchart LR
    sel[未排序区] --> scan[扫描找最小索引]
    scan --> swapSel[与区间首元素交换]
    swapSel --> shrink[未排序区缩小一格]
```
...
flowchart TB
    split[归并分半] --> left[递归左]
    split --> right[递归右]
    left --> mergeM[合并时左右指针取较小值]
    right --> mergeM
```

```mermaid
flowchart TB
    pivot[选 pivot] --> part[partition：< pivot 放左，> 放右]
    part --> leftQ[递归左区]
    part --> rightQ[递归右区]
    leftQ --> pivot
```
...
```mermaid
flowchart LR
    gap[gap = n/2] --> group[按 gap 分组做插入排序]
    group --> shrink[gap /= 2]
    shrink --> done{gap == 0?}
    done -- 否 --> gap
```
